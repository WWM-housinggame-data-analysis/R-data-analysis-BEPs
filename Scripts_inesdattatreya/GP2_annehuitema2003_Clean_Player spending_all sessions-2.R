...
for (nm in names(datasets_list)) {
  ...
  output_dir <- file.path("Users/inesdattatreya/Library/CloudStorage/OneDrive-DelftUniversityofTechnology/BEP_copy/BranchInes/Scripts_inesdattatreya/data_output/GS2_25-24_sessions/income_dis_satis_plot", 
...
# Sessions + datasets
# -----------------------------
### fix the missing satisfaction
fix_player_class_mode <- function(df, player_id = "t3p5", class_col = "class") {
  df %>%
    ...
    mutate(
      "{class_col}" := ifelse(
        ...
        {
          cls <- .data[[class_col]]
          # mode: meest voorkomende niet-NA class
          tab <- sort(table(cls, useNA = "no"), decreasing = TRUE)
          if (length(tab) == 0) NA else as.numeric(names(tab)[1])
        },
        .data[[class_col]]
      )
    ) %>%
...
sessions[["250923"]] <- fix_player_class_mode(sessions[["250923"]], "t3p5", "class")
### fix the missing satisfaction
sessions <- list(
...
output_base <- "/Users/inesdattatreya/Library/CloudStorage/OneDrive-DelftUniversityofTechnology/BEP_copy/BranchInes/Scripts_inesdattatreya/data_output/GS2_25-24_sessions/risk_ratio_satis_plot" 
...
# make a table to get an overview of how many classes and NA's there are every session

class_counts_per_session <- bind_rows(
  lapply(names(sessions), function(sess) {
    ...
    sessions[[sess]] %>%
      ...
      distinct(session, class, player_code) %>%   # 👈 unieke spelers
      count(session, class, name = "n_players")
  })
)

class_counts_wide <- class_counts_per_session %>%
  tidyr::pivot_wider(
    names_from  = class,
    values_from = n_players,
    values_fill = 0
  ) %>%
  arrange(session)
...
output_file <- file.path(
  output_base,
  "Class_counts_per_session_unique_players.xlsx"
)

write_xlsx(
  list(
    Class_counts_wide = class_counts_wide,
    Class_counts_long = class_counts_per_session
  ),
  output_file
)

message("✅ Saved class counts to:\n", output_file)
