welfare_spending_satisfaction_plot2 <- function(dataset,
  ...
  # satisfaction kan negatief zijn -> werk met absolute piek
  max_abs_sat <- max(abs(summary_df$ave_satisfaction), na.rm = TRUE)
  if (!is.finite(max_abs_sat) || max_abs_sat == 0) max_abs_sat <- 1
  
  # hoeveel van de linker-as hoogte mag de satisfaction lijn gebruiken?
  sat_frac <- 0.5   # <-- pas aan naar 0.5, 0.4, 0.6 etc
  
  # schaalfactor zodat max(|sat|) gelijk wordt aan sat_frac * max_cost
  scale_factor <- (sat_frac * max_cost) / max_abs_sat
  
  # geschaalde satisfaction op linker-as
  summary_df$ave_satisfaction_scaled <- summary_df$ave_satisfaction * scale_factor
  
  # Alleen als satisfaction negatief is, mag de linker as negatief worden
  y_left_min <- if (min(summary_df$ave_satisfaction, na.rm = TRUE) < 0) {
    min(summary_df$ave_satisfaction_scaled, na.rm = TRUE)
  } else {
    0
  }
  ...
  plot_title <- "Spending patterns and satisfaction per welfare level"
  plot_subtitle <- paste(
  ...
  plot <- ggplot() +
    ...
    theme_minimal(base_size = 14) +   # <-- basis alles groter
    theme(
      # TITELS
      ...
      axis.text.x         = element_markdown(size = 13, hjust = 0.5),
      axis.text.y         = element_text(size = 13),
      
      # RECHTER AS (satisfaction)
