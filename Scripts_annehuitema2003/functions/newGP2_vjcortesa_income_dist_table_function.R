...
")
  ...
  # ---- Build wide question answers per playerround_id ----
  qs_wide <- questionscore %>%
    mutate(question_name = trimws(question_name)) %>% 
    filter(question_name %in% c("Question 1.", "Question 2.")) %>%
    # If there is any chance of duplicates per playerround_id+question, keep the first
    group_by(playerround_id, question_name) %>%
    ...
    # Clean names so you don't get dots/spaces issues
    rename(
      answer_q1 = `answer_Question 1.`,
      answer_q2 = `answer_Question 2.`
    )
  
  # ---- Join into playerround ----
  playerround <- playerround %>%
    left_join(qs_wide, by = "playerround_id")
  
  table(is.na(qs_wide$answer_q1), is.na(qs_wide$answer_q2))
